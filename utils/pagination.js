...
paginate = async ({ totalItems, currentPage = 1, perPage = 8, adjacent = 2 }) => {
...
callPaginate = async (request, tableName) => {
    ...
    try {
        ...
        let items = await executeQuery(query + ' LIMIT ?, ?', [...params, offset, perPage]);
        ...
        if (items.length === 0 && currentPage > 1) {
            ...
            items = await executeQuery(query + ' LIMIT ?, ?', [...params, offset, perPage]);
...
// function generatePerPageOptions(currentPerPage, totalItems, incrementBy = 4) {
generatePerPageOptions = async (currentPerPage, totalItems, incrementBy = 4) => {

    let nextPage = currentPerPage;
    const perPageOptions = [{ value: currentPerPage, label: String(currentPerPage) }];
